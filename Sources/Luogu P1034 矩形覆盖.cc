...
void debug_output(PointStruct *a, PointStruct *b) {
	for (int i = 1; i <= rect_num; i++) {
		cout << a[i].rect.top << " " << a[i].rect.bottom << " " << a[i].rect.left << " " << a[i].rect.right << endl;
		cout << b[i].rect.top << " " << b[i].rect.bottom << " " << b[i].rect.left << " " << b[i].rect.right << endl;
	}
}
