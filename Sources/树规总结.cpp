...
int main()
{
 ...
 　我们继续扩展：如果上一道题目继续提问，要求输出所选的方案呢？
  ...
  　　既然数是递归定义的，所以我们依旧使用递归的形式来记录路径：使用一个bool数组ans来进行递归，分两种情况：取(1)和不取(0)。然后，我们继续利用已经求得的f[i][j]的值来思考如何找到路径：首先定义一个path()函数。如果f[i][j]=f[b[i]][j],那么节点i必然没有取，让ans[i]=0;否则，节点i一定取到了。（为什么呢？其实，这是依照第一问的dfs来思考的，第一问的dfs是这样定义的，所以我们就这样考虑了。）然后依照上一问，if(f[x][y]==f[b[x]][k-1]+f[c[x]][y-k]+s[x])，那么我们在i节点后选的一定是以上的方案，在这时让ans[i]=1,继续深搜path()即可。最后从1到n依次输出取到的点即可。
...
void merge()//合点
{
	...
	for (int i = 1; i <= tmpn; i++)
		for (int j = 1; j <= tmpn; j++)
		{
			...
			if (w[d[j]]<0 && w[j]>0 && mapp[j][d[j]] == 1 && /*mapp[j][d[j]]==1*/mapp[j][d[j]] == 1)
			{
				w[n - w[d[j]]] += w[j];//找到df所在的新点
				v[n - w[d[j]]] += v[j];//这里类似于并查集,任何一个合并过的点,都应该指向(得到新点的位置)相同的点,保证这些点都附属于同一个点,相当于一个点代表了一个环
				w[j] = w[d[j]];//类似并查集操作
			}
